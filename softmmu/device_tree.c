                             ...
                             const char *property, int *lenp, Error **errp)
{
    ...
    if (!r) {
        ...
        if (!qemu_devtree_getparent(fdt, parent, node_path)) {
            return qemu_fdt_getprop(fdt, parent, property, lenp, errp);
        }
                                    ...
                                    Error **errp)
{
    ...
    prop = qemu_fdt_getprop(fdt, node_path, property, &len, errp);
    if (*errp) {
        return NULL;
    }
    while (cell) {
                                     ...
                                     int size, Error **errp)
{
    ...
    for (; size; size--) {
        ret <<= 32;
        ret |= qemu_fdt_getprop_cell(fdt, node_path, property, offset++, errp);
        if (errp && *errp) {
            return 0;
        }
    }
...
uint32_t qemu_fdt_check_phandle(void *fdt, const char *path, Error **errp)
{
    uint32_t r;

    r = fdt_get_phandle(fdt, findnode_nofail(fdt, path));
    if (r == 0) {
        error_setg(errp, "%s: Couldn't find phandle for %s: %s", __func__,
                 path, fdt_strerror(r));
    }
...
const char *qemu_devtree_get_node_name(void *fdt, const char *node_path)
{
    return fdt_get_name(fdt, fdt_path_offset(fdt, node_path), NULL);
}
