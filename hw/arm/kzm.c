...
static void kzm_init(MachineState *machine)
{
    ...
    memory_region_add_subregion(&machine->main_system_bus.memory.mr, FSL_IMX31_SDRAM0_ADDR,
                                machine->ram);
    ...
    if (serial_hd(2)) { /* touchscreen */
        serial_mm_init(&machine->main_system_bus.memory.mr, KZM_FPGA_ADDR+0x10, 0,
                       qdev_get_gpio_in(DEVICE(&s->soc.avic), 52),
