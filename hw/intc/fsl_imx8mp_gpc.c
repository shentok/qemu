...
static const char *fsl_imx8mp_gpc_reg_name(uint32_t reg)
{
    switch (reg) {
                                    ...
                                    unsigned size)
{
    ...
    trace_fsl_imx8mp_gpc_read(offset, fsl_imx8mp_gpc_reg_name(idx), value);

    return value;
}

static void fsl_imx8mp_gpc_write(void *opaque, hwaddr offset,
                                 uint64_t value, unsigned size)
{
    FslImx8mpGpcState *s = opaque;
    const size_t idx = offset / sizeof(uint32_t);

    trace_fsl_imx8mp_gpc_write(offset, fsl_imx8mp_gpc_reg_name(idx), value);
...
static const struct MemoryRegionOps fsl_imx8mp_gpc_ops = {
    .read = fsl_imx8mp_gpc_read,
    .write = fsl_imx8mp_gpc_write,
...
static void fsl_imx8mp_gpc_reset(DeviceState *dev)
{
    FslImx8mpGpcState *s = IMX8MP_GPC(dev);
...
static void fsl_imx8mp_gpc_set_irq(void *opaque, int irq, int level)
{
    trace_fsl_imx8mp_gpc_set_irq(irq, level);
}

static void fsl_imx8mp_gpc_request_wake_gic(void *opaque, int irq, int level)
{
    FslImx8mpGpcState *s = opaque;

    trace_fsl_imx8mp_gpc_request_wake(irq, level);
...
static void fsl_imx8mp_gpc_wfi(void *opaque, int irq, int level)
{
    ...
    trace_fsl_imx8mp_gpc_wfi(irq, level);
...
static void fsl_imx8mp_gpc_init(Object *obj)
{
    ...
    FslImx8mpGpcState *s = IMX8MP_GPC(obj);

    memory_region_init_io(&s->iomem,
                          obj,
                          &fsl_imx8mp_gpc_ops,
                          s,
                          TYPE_IMX8MP_GPC ".iomem",
                          sizeof(s->regs));
    sysbus_init_mmio(sd, &s->iomem);
    qdev_init_gpio_in(DEVICE(sd), fsl_imx8mp_gpc_set_irq, 160);
    qdev_init_gpio_in_named(DEVICE(sd), fsl_imx8mp_gpc_request_wake_gic,
                            IMX8MP_GPC_REQUEST_WAKE_GIC, 4);
    qdev_init_gpio_in_named(DEVICE(sd), fsl_imx8mp_gpc_wfi, IMX8MP_GPC_WFI, 4);
}

static const VMStateDescription vmstate_fsl_imx8mp_gpc = {
    .name = TYPE_IMX8MP_GPC,
    .version_id = 1,
    .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_UINT32_ARRAY(regs, FslImx8mpGpcState, IMX8MP_GPC_NUM),
        VMSTATE_END_OF_LIST()
    },
};

static void fsl_imx8mp_gpc_class_init(ObjectClass *klass, const void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    device_class_set_legacy_reset(dc, fsl_imx8mp_gpc_reset);
    dc->vmsd  = &vmstate_fsl_imx8mp_gpc;
    dc->desc  = "i.MX GPCv2 Module";
}

static const TypeInfo fsl_imx8mp_gpc_info = {
    .name          = TYPE_IMX8MP_GPC,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(FslImx8mpGpcState),
    .instance_init = fsl_imx8mp_gpc_init,
    .class_init    = fsl_imx8mp_gpc_class_init,
};

static void fsl_imx8mp_gpc_register_type(void)
{
    type_register_static(&fsl_imx8mp_gpc_info);
}
type_init(fsl_imx8mp_gpc_register_type)
