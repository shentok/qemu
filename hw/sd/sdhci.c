...
uint64_t sdhci_read(SDHCIState *s, hwaddr offset, unsigned size)
{
    uint32_t ret = 0;
...
void sdhci_write(SDHCIState *s, hwaddr offset, uint64_t val, unsigned size)
{
...
static uint64_t sdhci_sysbus_read(void *opaque, hwaddr offset, unsigned size)
{
    SDHCIState *s = (SDHCIState *)opaque;

    return sdhci_read(s, offset, size);
}

static void
sdhci_sysbus_write(void *opaque, hwaddr offset, uint64_t val, unsigned size)
{
    SDHCIState *s = (SDHCIState *)opaque;

    sdhci_write(s, offset, val, size);
}

static const MemoryRegionOps sdhci_mmio_ops = {
    .read = sdhci_sysbus_read,
    .write = sdhci_sysbus_write,
    .valid = {
        .min_access_size = 1,
        .max_access_size = 4,
        .unaligned = false
