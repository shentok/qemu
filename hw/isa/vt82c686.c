...
static void via_pm_get_prop_gpe0_blk(Object *obj, Visitor *v,
                                     const char *name, void *opaque,
                                     Error **errp)
{
    ViaPMState *s = VIA_PM(obj);
    uint64_t value = s->io_gpe_qemu.addr;

    visit_type_uint64(v, name, &value, errp);
}

static void via_pm_get_prop_gpe0_blk_len(Object *obj, Visitor *v,
                                         const char *name, void *opaque,
                                         Error **errp)
{
    ViaPMState *s = VIA_PM(obj);
    uint64_t value = s->ar.gpe.len;
