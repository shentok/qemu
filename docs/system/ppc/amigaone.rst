...
From the firmware menu that appears select ``Boot sequence -> Amiga Multiboot Options``
and set ``Boot device 1`` to ``VIA CDROM``. Then hit escape until the main screen appears again, hit escape once more and from the exit menu that appears select either ``save settings`` or ``use settings for current boot``. It may take a long time loading the kernel into memory but eventually it boots and the installer becomes visible.
